...
class VKinder:
    ...
    def name(self, user_id):
        url = f'https://dev.vk.com/method/users.get'
    ...
    def get_age_low(self, user_id):
        url = url = f'https://dev.vk.com/method/users.get'
        params = {'access_token': user_token,
                  'user_ids': user_id,
                  'fields': 'bdate',
                  'v': '5.131'}
        ...
        try:
            ...
            if len(date_list) == 3:
                ...
                self.write_msg(user_id, 'Минимальный возраст (min - 16): ')
    ...
    def get_age_high(self, user_id):
        ...
        try:
            ...
            if len(date_list) == 3:
                ...
                self.write_msg(user_id, 'Максимальный возраст (max - 65): ')
                for event in self.longpoll.listen():
                    if event.type == VkEventType.MESSAGE_NEW and event.to_me:
                        age = event.text
                        return age
        except KeyError:
            self.write_msg(user_id, 'Ошибка получения токена, введите токен - user_token')

    # @staticmethod
    def cities(self, user_id, city_name):
        url = url = f'https://dev.vk.com/method/database.getCities'
        params = {'access_token': user_token,
                  'country_id': 1,
                  'q': f'{city_name}',
                  'need_all': 0,
        ...
        try:
            ...
            for i in information_dict:
                if 'city' in i:
                    ...
                    self.write_msg(user_id, 'Название города: ')
                    for event in self.longpoll.listen():
                        if event.type == VkEventType.MESSAGE_NEW and event.to_me:
    ...
    def find_user(self, user_id):
        url = f'https://dev.vk.com/methodusers.search'
        params = {'access_token': user_token,
                  'v': '5.131',
    ...
    def get_photos_id(self, user_id):
        url = 'https://dev.vk.com/method/photos.getAll'
        params = {'access_token': user_token,
                  'type': 'album',
                  'owner_id': user_id,
                  'extended': 1,
